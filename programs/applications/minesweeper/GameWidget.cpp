...
GameWidget::GameWidget() {
    ...
    auto bomb_file = Gfx::Image::load(base_path / "bomb.png");
    if (bomb_file.is_error()) {
        Duck::Log::errf("bomb file error: {}", bomb_file.result());
    }
    bomb = bomb_file.value();
    reset();
}
        ...
        for (int j = 0; j < board.rows(); j++) {
            if (!board.is_visible(i, j)) {
                ...
                if (board.is_marked(i, j)) {
                    ...
                    ctx.draw_image(bomb, {i * (CELL_SIZE + 1) + (CELL_SIZE - IMAGE_SIZE) / 2, j * (CELL_SIZE + 1) + (CELL_SIZE - IMAGE_SIZE) / 2});
                } else {
