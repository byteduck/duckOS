...
int Display::millis_until_next_flip() const {
	...
	int diff =
			(int) (((new_time.tv_sec - paint_time.tv_sec) * 1000000) + (new_time.tv_usec - paint_time.tv_usec)) / 1000;
...
void Display::create_mouse_events(int delta_x, int delta_y, int scroll, uint8_t buttons) {
	...
	for (auto it = _windows.rbegin(); it != _windows.rend(); it++) {
		...
		if (!_resize_window && window->resizable() &&
	...
	if (event_window != _prev_mouse_window && _prev_mouse_window != nullptr &&
		!_prev_mouse_window->gets_global_mouse()) {
		...
		Gfx::Point new_mouse_pos = (mouse - _prev_mouse_window->absolute_rect().position()).constrain(
				{0, 0, window_dims.width, window_dims.height});
		_prev_mouse_window->mouse_moved(delta, new_mouse_pos,
										new_mouse_pos + _prev_mouse_window->absolute_rect().position());
...
ResizeMode Display::get_resize_mode(Gfx::Rect window, Gfx::Point mouse) {
	if (mouse.in({window.x - WINDOW_RESIZE_BORDER, window.y - WINDOW_RESIZE_BORDER, WINDOW_RESIZE_BORDER * 3,
				  WINDOW_RESIZE_BORDER * 3}))
		return NORTHWEST;
	if (mouse.in({window.x + window.width - WINDOW_RESIZE_BORDER * 2, window.y - WINDOW_RESIZE_BORDER,
				  WINDOW_RESIZE_BORDER * 3, WINDOW_RESIZE_BORDER * 3}))
		return NORTHEAST;
	if (mouse.in({window.x - WINDOW_RESIZE_BORDER, window.y + window.height - WINDOW_RESIZE_BORDER * 2,
				  WINDOW_RESIZE_BORDER * 3, WINDOW_RESIZE_BORDER * 3}))
		return SOUTHWEST;
	if (mouse.in(
			{window.x + window.width - WINDOW_RESIZE_BORDER * 2, window.y + window.height - WINDOW_RESIZE_BORDER * 2,
			 WINDOW_RESIZE_BORDER * 3, WINDOW_RESIZE_BORDER * 3}))
		return SOUTHEAST;
